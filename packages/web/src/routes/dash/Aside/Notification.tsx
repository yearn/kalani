...
export default function Notification({
  ...
  actions?: ReactNode, children?: React.ReactNode
}) {
  return <div className={`pl-8 pr-3 pt-8 pb-3 flex flex-col gap-8 border-primary border-warn-950 rounded-primary text-warn-400`}>
    <div className="flex items-center gap-8 text-warn-400">
      {children}
    </div>
...
export function FixItNotification({
  id, icon: Icon, authorized, onFix, actions, children
}: {
  id: string, icon: IconType, authorized: boolean, onFix?: () => void, actions?: ReactNode, children?: React.ReactNode
}) {
